...
def depthFirstSearch(problem):
    ...
    constStart = problem.getStartState()
    explored = []
    explored.append(start)
    stack = util.Stack()
    stateTuple = (start, [])
    stack.push(stateTuple)
    while not stack.isEmpty() and not problem.isGoalState(constStart):
...
def uniformCostSearch(problem):
    ...
    priorQueue.push((startState, []) ,0)
    while not priorQueue.isEmpty():
        state, actions = priorQueue.pop()
        if problem.isGoalState(state):
            return actions
        if state not in explored:
            ...
            for succ in successors:
                ...
                if coordinates not in explored:
                    ...
                    priorQueue.push((coordinates, actions + [directions]), problem.getCostOfActions(newCost))
        explored.append(state)
    return actions
    util.raiseNotDefined()
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    explored = []
    priorQueue = util.PriorityQueue()
    priorQueue.push((startState, []), nullHeuristic(startState, problem))
    nCost = 0
    while not priorQueue.isEmpty():
        state, actions = priorQueue.pop()
        if problem.isGoalState(state):
        ...
        if state not in explored:
            ...
            for succ in successors:
                ...
                if coordinates not in explored:
                    ...
                    priorQueue.push((coordinates, actions + [directions]), nCost)
        explored.append(state)
    return actions
    util.raiseNotDefined()
